...
namespace JOIEnergy.Tests
{
    public class PricePlanComparisonTest
    {
        ...
        [Fact]
        public void GetUsageCostOfLastWeekSpending_InvalidSmartMeterId_ReturnZeoCost()
        {
            var pricePlanService = new PricePlanService(new List<PricePlan>(), meterReadingService);
            var energyConsumed = pricePlanService.GetUsageCostOfLastWeekSpending("No-Meter","");
            var result= new Dictionary<string, decimal>();
            Assert.Equal(energyConsumed, result);
        }
        [Fact]
        public void GetUsageCostOfLastWeekSpending_ValidSmartMeterId_ReturnCost()
        {
            var pricePlans = new List<PricePlan>() {
                new PricePlan() { PlanName = PRICE_PLAN_1_ID, UnitRate = 10, PeakTimeMultiplier = NoMultipliers() },
                new PricePlan() { PlanName = PRICE_PLAN_2_ID, UnitRate = 1, PeakTimeMultiplier = NoMultipliers() },
            ...
            accountService= new AccountService(new Dictionary<string, string>
            {
                { "smart-meter-0", PRICE_PLAN_1_ID }
            });
            var pricePlanService = new PricePlanService(pricePlans, meterReadingService);
            var smartMeterId = "smart-meter-0";
            meterReadingService.StoreReadings(smartMeterId, new List<ElectricityReading>
            {
                new ElectricityReading { Time = DateTime.Now.AddDays(-5), Reading = 100 },
                new ElectricityReading { Time = DateTime.Now, Reading = 200 }
            });

            var energyConsumed = pricePlanService.GetUsageCostOfLastWeekSpending(smartMeterId, PRICE_PLAN_1_ID);
            Assert.NotEmpty(energyConsumed); // Assuming unit rate is 10
        }
